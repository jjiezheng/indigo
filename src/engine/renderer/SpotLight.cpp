...
Matrix4x4 SpotLight::transform() const {
  ...
  return Matrix4x4::translation(position_) * rotation() * Matrix4x4::rotationX(90) * Matrix4x4::scale(Vector4(xzScale, length_, xzScale));
}
...
Matrix4x4 SpotLight::projection() const {
  return Matrix4x4::perspective(toDegrees(outerAngle_ / 2.0f), GraphicsInterface::aspectRatio(), 1.0f, 200.0f);
}

Matrix4x4 SpotLight::viewTransform() const {
  Matrix4x4 transform = Matrix4x4::translation(position_) * orientation_ * rotation();
